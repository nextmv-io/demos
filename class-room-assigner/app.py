...
options = nextmv.Options(
    nextmv.Option(name="app_id", option_type=str, default="class-assignment"),
    nextmv.Option(name="app_name", option_type=str, default="Class Assignment"),
...
if options.action == "sync" or options.action == "push":
    ...
    api_key = os.environ.get("NEXTMV_MARRIOTT_KEY")
    if api_key is None:
        raise Exception("Please set NEXTMV_MARRIOTT_KEY environment variable")
...
if options.action == "local":
    run_1 = local_app.new_run(nextmv.load())
    print(run_1)
