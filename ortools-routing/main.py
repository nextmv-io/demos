...
# Strategies exposed as options. These are the names of OR-Tools' enum values;
# we map them to the enum at solve time via getattr. The lists are a curated,
# demo-friendly subset -- any valid OR-Tools enum name also works.
...
def parameters() -> nextmv.Options:
    """Declares the tunable options (parameters) of the app."""
    return nextmv.Options(
        nextmv.Option(
            "duration",
            int,
            30,
            "Max solver runtime (in seconds).",
            False,
            additional_attributes={"min": 1, "max": 300, "step": 5},
            control_type="slider",
        ...
        nextmv.Option(
            ...
            str,
            "AUTOMATIC",
            "OR-Tools first-solution strategy (builds the initial routes).",
            False,
            choices=FIRST_SOLUTION_STRATEGIES,
            control_type="select",
        ...
        nextmv.Option(
            ...
            str,
            "AUTOMATIC",
            "OR-Tools local-search metaheuristic (improves the routes).",
            False,
            choices=LOCAL_SEARCH_METAHEURISTICS,
            control_type="select",
        ...
        nextmv.Option(
            ...
            bool,
            False,
            "Log OR-Tools search progress to stderr.",
            False,
...
def main() -> None:
    ...
    loaded_input = nextmv.load(options=parameters())
    ...
    # Emit BOTH a flat `metrics` dict (read by scenario/acceptance tests) and a
    # canonical Nextmv `statistics` object (read by ensemble rules via
    # statistics_path). Ensemble rules query `result.value`, so set it to the
    # objective when feasible and to a large sentinel when not — that way every
    # run carries the stat (the rule never errors) and an infeasible config can
    # never win the "minimize objective" selection.
    feasible = metrics.get("solution_found", False)
    infeasible_sentinel = 1e12
    statistics = nextmv.Statistics(
        run=nextmv.RunStatistics(duration=metrics.get("duration", 0.0)),
        result=nextmv.ResultStatistics(
            value=metrics["objective_value"] if feasible else infeasible_sentinel,
...
def solve(input: nextmv.Input, options: nextmv.Options) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    # Configure the search. The first-solution strategy builds the initial
    # routes; the local-search metaheuristic then improves them until the time
    # limit is hit. Both are exposed as options so they can be swept in a
    # scenario test and compared in an ensemble -- the heart of this demo.
    search_parameters = pywrapcp.DefaultRoutingSearchParameters()
    search_parameters.time_limit.FromSeconds(options.duration)
    search_parameters.first_solution_strategy = resolve_enum(
        routing_enums_pb2.FirstSolutionStrategy,
        options.first_solution_strategy,
        "first_solution_strategy",
    )
    search_parameters.local_search_metaheuristic = resolve_enum(
        routing_enums_pb2.LocalSearchMetaheuristic,
        options.local_search_metaheuristic,
        "local_search_metaheuristic",
    )
...
def resolve_enum(enum_wrapper: Any, name: str, option_name: str) -> int:
    """Maps an OR-Tools enum value name (e.g. "SAVINGS") to its integer value.

    Raises a clear error if the name is not a valid value for the enum, so a
    typo in a scenario test fails loudly instead of silently using a default.
    """
    try:
        return getattr(enum_wrapper, name)
    except AttributeError as exc:
        raise ValueError(f"Invalid value '{name}' for option '{option_name}'.") from exc
