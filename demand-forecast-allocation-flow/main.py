...
class DemandAllocationFlow(FlowSpec):
    @step
    def forecast_demand(flow_input: dict) -> dict:
        """
        Train a GradientBoostingRegressor on historical sales data and predict
        demand for the next period at each store.

        Returns a dict that doubles as the input payload for the
        inventory-allocation sub-app (explicit demand/supply mode).
        """
        import pandas as pd
        ...
        for store in stores:
            ...
            nextmv.log(
                f"  {store['name']}: period {next_period} forecast = {predicted} units"
            )

        supply = [w["supply"] for w in flow_input["warehouses"]]
        nextmv.log(
            f"Total forecasted demand: {sum(demand)}, total supply: {sum(supply)}"
        )
    ...
    @step
    def run_allocation():
        """Delegates to the inventory-allocation cloud app.

        Nextpipe passes the output of forecast_demand as the sub-app input,
        which inventory-allocation reads in explicit demand/supply mode.
        """
        pass  # Execution happens in the sub-app.

    @needs(predecessors=[forecast_demand, run_allocation])
    @step
    def bundle_results(forecast_output: dict, allocation_result: dict):
        """Combine forecast and allocation outputs into visualizations."""

        allocation_solution = allocation_result.get("solution", {})
        allocation_metrics = (
            allocation_result.get("statistics", {}).get("result", {})
        )
...
def main() -> None:
    ...
    nextmv.write(flow.get_result(flow.bundle_results), path=options.output)
